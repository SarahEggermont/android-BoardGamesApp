 ...
 */
fun CafeApiService.getCafesAsFlow(): Flow<List<ApiCafe>> =
    flow {
 ...
 * @return a [Flow] containing a [List] of [ApiCafe]s.
 */
fun CafeApiService.getCafesSearchAsFlow(search: String): Flow<List<ApiCafe>> =
    flow {
        emit(
            getCafesSearch(
 ...
 */
fun CafeApiService.getCafeAsFlow(name: String): Flow<ApiCafe> =
    flow {
        emit(
            getCafe(
